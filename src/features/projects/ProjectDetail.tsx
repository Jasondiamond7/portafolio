...
import { AutomationPipelineBackground } from './AutomationPipelineBackground'
import { ProjectMetricsChart } from './ProjectMetricsChart'

export function ProjectDetail({ project }: { project: Project }) {
  return (
    <div className="relative overflow-hidden bg-slate-50 dark:bg-slate-900/40">
      {project.background ? (
        <div
          className={
            project.background === 'automation'
              ? 'pointer-events-none absolute inset-0 opacity-35'
              : 'pointer-events-none absolute inset-0 opacity-70'
          }
        >
          {project.background === 'automation' ? (
            <AutomationPipelineBackground />
          ) : (
